...
class SynNLI_Trainer:
    ...
    def eval_model_when_training(self, model, dev_set):
        ...
        dev_loader = DataLoader(dev_set, batch_size=config.BATCH_SIZE, follow_batch=config.follow_batch)
        pred = []
        label = []
        ...
        for batch_i, batch in enumerate(tqdm(dev_loader)):
            ...
            for k in config.tensor_attr_list:
                    batch.__dict__[k] = batch.__dict__[k].to(self.device)
    ...
    # config.follow_batch is dominant now
    def train(self, train_set, dev_set, word2idx, pretrained_word_vectors, model_file_path, batch_size, follow_batch=config.follow_batch, log_stream=None):
        """
        trainset/devset = set or 
        """
        # set saving directory
        if model_file_path == None:
            if config.LOG:
                print("no specified save path, set to default, default is " + config.SAVE_MODEL_FOLDER, file=log_stream)
        ...
        if train_set == None:
            if(config.LOG):
                print('loading trainset from {}'.format(config.PTRAIN_FILE), file=log_stream)
            train_set = GraphDataset(config.PTRAIN_FILE, word2idx=word2idx, to_lower=True)
        elif isinstance(train_set, str):
            train_set = GraphDataset(train_set, word2idx=word2idx, to_lower=True)
        elif isinstance(train_set, GraphDataset):
            pass
        elif isinstance(train_set, list):
            logging.info("train_set is in list[Data] form")
        elif isinstance(train_set, GraphDataset):
            logging.info("train_set is in " + str(type(train_set)) + " form")
        else:
            logging.error("train_set is of type " + str(type(train_set)))
            logging.error("critical error: set is not in desired type")
            assert(False)
            
        # dev set
        if dev_set == None:
            if(config.LOG):
                print('loading devset from {}'.format(config.PDEV_MA_FILE), file=log_stream)
            dev_set = GraphDataset(config.PDEV_MA_FILE, word2idx=word2idx, to_lower=True)
        elif isinstance(dev_set, str):
            dev_set = GraphDataset(dev_set, word2idx=word2idx, to_lower=True)
        elif isinstance(dev_set, GraphDataset):
            pass
        elif isinstance(dev_set, list):
            logging.info("dev_set is in list[Data] form")
        elif isinstance(dev_set, GraphDataset):
            logging.info("dev_set is in " + str(type(dev_set)) + " form")
        else:
            logging.error("critical error: set is not in desired type")
            assert(False)
        dev_loader = DataLoader(dev_set, batch_size=batch_size, follow_batch=config.follow_batch)
        train_loader = DataLoader(train_set, batch_size=batch_size, follow_batch=config.follow_batch)
        ...
        if self.model == None:
            ...
            self.model = SynNLI_Model(nli_config=config.nli_config, pretrained_embedding_tensor=pretrained_word_vectors)
