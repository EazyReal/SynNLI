...
@TokenIndexer.register("elmo_characters_with_mask", exist_ok=True)
class ELMoTokenCharactersIndexerWithMask(TokenIndexer):
    """
    ...
    def as_padded_tensor_dict(
        ...
        # referecne the code from pretrained transformer matched indexer
        # print(tokens, padding_lengths)
        # print(len(tokens["elmo_tokens"]), len(tokens["elmo_tokens"][0])) 4, 50
        tensor_dict["mask"] = torch.BoolTensor(
            pad_sequence_to_length(
                [True for i in range(len(tokens["elmo_tokens"]))], padding_lengths["elmo_tokens"], default_value=lambda: False
            )
        )
